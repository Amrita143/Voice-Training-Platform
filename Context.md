...
5. **CI/CD set up (2026-06-18)** — see **`DEPLOY.md`**. `git push` to `main` →
   (a) GitHub Actions (`.github/workflows/deploy.yml`) builds + deploys **web + firestore rules** to Firebase (needs repo secret `FIREBASE_SERVICE_ACCOUNT`); (b) **Render** (`render.yaml`) auto-deploys the **proxy** (`server/`, set `XAI_API_KEY` in Render). Public Firebase web config moved into committed `apps/web/.env.production` so CI can build; set `VITE_API_BASE` there to the Render proxy URL once hosted. CI build verified locally (`build:shared && build:web`). **Still TODO by user:** push to GitHub, add the SA secret, create the Render service + set `VITE_API_BASE`.
